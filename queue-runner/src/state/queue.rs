...
impl Queues {
    ...
    pub async fn insert_new_jobs_into_fod<S: Into<String> + std::fmt::Debug>(
        ...
        let _ = wq.insert_new_jobs(system, jobs, now, sort_fn);
    }
    ...
    pub async fn remove_all_weak_pointer(&self, hint: Option<QueueType>) {
        let arr = match hint {
            Some(v) => match v {
                ...
                QueueType::Fod => vec![&self.fod],
            },
            None => vec![&self.main, &self.fod],
        };
...
mod tests {
    ...
    async fn test_job_constraint_scheduling_fod() {
        ...
        queues
            ...
            .await;
        queues
            .insert_new_jobs_into_fod("x86_64-linux", vec![job1, job3], &now, StepSortFn::Legacy)
            .await;
