...
pub struct Queues {
    main: tokio::sync::RwLock<InnerQueues>,
    fod: tokio::sync::RwLock<InnerQueues>,
}
...
impl Queues {
    ...
    pub fn new() -> Self {
        Self {
            main: tokio::sync::RwLock::new(InnerQueues::new(QueueType::Main, vec![])),
            fod: tokio::sync::RwLock::new(InnerQueues::new(
                QueueType::Fod,
    ...
    #[must_use]
    pub fn with_features(main_features: Vec<String>, fod_features: Vec<String>) -> Self {
        Self {
            main: tokio::sync::RwLock::new(InnerQueues::new(QueueType::Main, main_features)),
            fod: tokio::sync::RwLock::new(InnerQueues::new(QueueType::Fod, fod_features)),
        }
    }
    ...
    pub async fn insert_new_jobs_into_main<S: Into<String> + std::fmt::Debug>(
        ...
        let mut wq = self.main.write().await;
    ...
    pub async fn insert_new_jobs_into_fod<S: Into<String> + std::fmt::Debug>(
        ...
        let mut wq = self.fod.write().await;
        let sort_duration = wq.insert_new_jobs(system, jobs, now, sort_fn);
        metrics.queue_sort_duration_ms_total.inc_by(sort_duration);
    }
    ...
    #[tracing::instrument(skip(self))]
    pub async fn clone_inner(&self, queue_type: QueueType) -> AHashMap<System, Arc<BuildQueue>> {
        let rq = match queue_type {
            QueueType::Main => self.main.read().await,
            QueueType::Fod => self.fod.read().await,
        };
    ...
    pub async fn get_stats_per_queue(
        &self,
        queue_type: QueueType,
    ) -> (&'static str, AHashMap<System, BuildQueueStats>) {
        let rq = match queue_type {
            QueueType::Main => self.main.read().await,
            QueueType::Fod => self.fod.read().await,
        };
...
mod tests {
    ...
    fn create_dummy_step_info(name: &str, system: &str, features: &[String]) -> StepInfo {
        let drv_path = create_dummy_store_path(name);

        let step = crate::state::Step::new(drv_path.clone());
