...
pub struct QueueRunnerStats {
    ...
    main_queue_stats: AHashMap<crate::state::System, BuildQueueStats>,
    #[serde(skip_serializing_if = "Option::is_none")]
    fod_queue_stats: Option<AHashMap<crate::state::System, BuildQueueStats>>,
...
impl QueueRunnerStats {
    pub async fn new(state: Arc<crate::state::State>) -> Self {
        ...
        let fod_queue_stats = if state.config.get_enable_fod_checker() {
            Some(
                state
                    .queues
                    .get_stats_per_queue(QueueType::Fod)
                    .await
                    .1
                    .into_iter()
                    .map(|(system, stats)| (system, stats.into()))
